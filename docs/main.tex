...
\begin{algorithm}[H]
\caption{Random Swapping}\label{Random Algorithm}
\begin{algorithmic}

\While{Less than 2 seconds has passed}
    \State $currOrder \gets $ random permutation of the $n$ nodes

    \State $steps \gets 0$ \Comment{\textcolor{blue}{keeps track of the number of times cities has been compared without any improvements}}
...
similar, but will have a higher constant factor since it will iterate through more nodes and checks. For the same reason, 2-opt is in some cases more efficient compared to 3-opt, since if three edges are considered, there are many more ways to replace the edges.


 algorithm that requires $O(n)$. however for each swap, it takes $O(n)$. run until 2 seconds has gone by.
